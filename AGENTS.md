# 全胶囊（theme-pill）——LinkDesk 插件仓

> **本文件是给在这个仓里干活的 AI 看的**（Claude Code / Codex / Cursor / …）。人看 `README.md`。
> 插件身份的唯一来源 = `plugin.json` 顶层的 `pluginId`（本仓：`theme-pill`）。当前版本 `1.0.2`。

## 1. 这是什么

全胶囊——把 `radius` 域拉满：圆角八档全部 999px，配合悬浮形态，泡泡糖配色。
